...
const ContactCTA: React.FC = () => {
  return (
    <section className="py-24 px-6 md:px-12 bg-charcoal-900 text-white">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 max-w-3xl mx-auto">
          {[
            ...
            { icon: Linkedin, label: 'LinkedIn', value: 'linkedin.com/in/vishal-rathod-productdesigner', href: 'https://linkedin.com/in/vishal-rathod-productdesigner' },
            ...
            <motion.a 
                ...
                className={`p-6 rounded-3xl bg-white/5 border border-white/10 flex items-center gap-4 ${item.href ? 'hover:bg-white/10 hover:border-blue-500/50 transition-all duration-300' : ''}`}
            >
              <item.icon className="w-8 h-8 text-blue-400" />
              <div>
                <p className="text-neutral-400 text-sm">{item.label}</p>
                <p className="font-bold text-white text-base break-all">{item.value}</p>
              </div>
            </motion.a>
          ))}
        </div>
